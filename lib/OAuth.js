...
OAuth.prototype = {
  wxLogin: function (req, res, next) {
    ...
    if ( this._isObject( req.query ) && typeof req.query.oauthredirect === 'string' ) {
      ...
      // Skip when it is not a WeChat browser
      if (this.noWechat === true) {
        fs.createReadStream(__dirname + '/../static/no_wechat.html').pipe(res)
      } else if (typeof this.noWechat === 'string') {
        fs.createReadStream(this.noWechat).pipe(res)
      } else {
        next()
      }
      ...
      this.getAccessToken(code, function (err, body) {
        ...
        if (body.errcode) {
          ...
          if ( errcode === 40029 || errcode === 40163 ) {
            ...
            return that.redirect(res, oauthUrl)
          } else {
            return next(err)
          }
        }
