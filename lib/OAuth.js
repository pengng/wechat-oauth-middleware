      ...
      this.getAccessToken(code, function (err, body) {
        ...
        that.getUser(openid, access_token, function (err, body) {
          ...
          if (url) {
            ...
            var urlJson = urlParser.parse(url, true)
            for (var key in body) {
              urlJson.query[key] = body[key]
            }
            urlJson.query.access_token = access_token
            var newUrl = urlJson.protocol + '//' + urlJson.host + urlJson.pathname + '?' + querystring.stringify(urlJson.query) + '#' + urlJson.hash
